...
지금은 `simple_vehicle.py` 하나에 차량 모델(조립)·제어(조향/디퍼렌셜/타이어력)·동역학 스텝핑이 다 얽혀 있음. 나중에 이걸 3단으로 분리하고 싶음:

- **모델**: URDF로 링크-조인트 토폴로지(+관성)만 분리. 단, URDF는 순수 기구학 트리라 스프링/디퍼렌셜/타이어력 같은 로직은 못 담음 — 그건 결국 동역학 레이어에 남음.
- **동역학**: FMU로 래핑. 두 가지 경로:
  1. **`pythonfmu`** (추천, 먼저 시도): 순수 Python 라이브러리로 `Fmi2Slave`를 상속해서 `do_step()` + 입출력 변수만 정의하면 표준 `.fmu`로 패키징됨. 지금 PyChrono 시뮬레이션 루프를 그대로 감싸면 되고, Chrono/C++을 새로 빌드할 필요 없음. 단, 실행 쪽에 Python 런타임이 있어야 함.
  2. **Chrono 네이티브 `chrono_fmi` 모듈**: conda `pychrono` 바이너리엔 없고, Chrono를 소스에서 C++로 직접 빌드(CMake + FMI 모듈 옵션 켬)해야 나옴. Python 런타임 의존성 없는 진짜 네이티브 FMU가 나오지만 진입장벽 훨씬 높음.
- **제어**: ros_control 스타일 — `hardware_interface`처럼 조인트 상태/커맨드를 좁은 인터페이스로 노출하고, 조향/디퍼렌셜/타이어력 같은 로직을 컨트롤러 플러그인으로 분리해서 물리 스텝핑 루프와 갈라놓기.

트레이드오프: 이 분리는 ROS2/Simulink 등 외부 툴과 실제로 연동할 때 값어치가 있고, 계속 이 레포 안에서만 쓸 거면 지금 구조 대비 초기 비용이 큼. 착수 전 어디부터 쪼갤지(URDF 토폴로지 추출 먼저 vs FMU 래핑 먼저) 정하고 시작할 것.
